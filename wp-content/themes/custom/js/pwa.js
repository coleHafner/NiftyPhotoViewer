...
//Global variables
var username = 'colehafner'; 
var photosize = '800'; 
var columns='4';

var columns = ( !columns || isNaN(columns) || columns < 1 ) ? columns = 4 : columns;
var base_url_entry = 'http://picasaweb.google.com/data/entry/base/user/' + username;
var base_url_feed = 'http://picasaweb.google.com/data/feed/base/user/' + username;
var photosize = ( !photosize ) ? photosize = 800 : photosize;
var callback_function = 'picasaweb';
...
if( go )
{
	...
	if( _GET['photoid'] &&
		...
		callback_function = "getphotolist";
		...
		$('<script type="text/javascript" src="' + base_url_feed + '/albumid/'+_GET['albumid']+'?category=photo&alt=json&callback=' + callback_function + '"></script>');
		
		callback_function = "photo";
		
		//if we have photo id AND album id, display current photo
		$('<script type="text/javascript" src="' + base_url_entry + '/albumid/'+_GET['albumid']+'/photoid/'+_GET['photoid']+'?alt=json&callback=' + callback_function + '"></script>');
	
	}
	else if( _GET['albumid'] && 
			!_GET['photoid'] ) {
		
		callback_function = "albums";
		
		//if we have album id, but no photo id, list all photos in current album
		$('<script type="text/javascript" src="' + base_url_feed + '/albumid/'+_GET['albumid']+'?category=photo&alt=json&callback=' + callback_function + '"></script>');
		
	} else {
	
		//if we have neither photo or album id, list all albums
		$('<script type="text/javascript" src="' + base_url_feed + '?category=album&alt=json&callback=' + callback_function + '&access=public"></script>');
	}
}
